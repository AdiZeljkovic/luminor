...
export default function FAQEditPage() {
    ...
    useEffect(() => {
        const fetchFAQ = async () => {
            try {
                ...
                if (response.success) {
                    const { question_en, question_bs, answer_en, answer_bs, category, order_num, is_active } = response.data;
                    setForm({ question_en, question_bs, answer_en, answer_bs, category, order_num, is_active });
                } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await apiClient.put(`/api/faq/${id}`, form);
            if (response.success) {
    ...
    return (
        <div className="space-y-8 animate-fade-up max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between bg-white border border-gray-200 p-6 rounded-2xl shadow-sm">
                <div>
                    <h1 className="text-3xl font-extrabold font-display text-[#0F172A]">Edit FAQ</h1>
                    <p className="text-gray-500 font-medium mt-1">Update the frequently asked question.</p>
                </div>
